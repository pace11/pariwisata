                            ...
                            <div class="table-responsive">
                                <table id="example1" class="table table-bordered table-striped">
                                    ...
                                    <tbody>
                                        <?php
                                        $no = 1;
                                        $sql = mysqli_query($conn,"SELECT * FROM tbl_pariwisata WHERE SUBSTRING(id_kategori,1,2)='OW'") or die (mysqli_error($conn));
                                        while($data = mysqli_fetch_array($sql)){ ?>
                                            <tr>    
                                                <td><?= $no ?></td>
                                                <td><img src="img/<?= $data['nama_foto'] ?>" width=100%"></td>
                                                <td><a class="btn btn-success btn-sm" href="#"><?= $data['id_kategori'] ?></a></td>
                                                <td><a class="btn btn-success btn-sm" href="#"><?= $data['id_daerah'] ?></a></td>
                                                <td><?= $data['nama'] ?></td>
                                                <td><?= $data['alamat'] ?></td>
                                                <td><?= $data['latitude'] ?></td>
                                                <td><?= $data['longitude'] ?></td>
                                                <td>
                                                <a class="btn btn-warning btn-sm" href="?page=objekwisatalihat&id=<?php echo $data['id_kategori']; ?>"><i class="fas fa-eye"></i> Lihat</a>
